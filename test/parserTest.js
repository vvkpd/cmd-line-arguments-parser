...
test["setHelpFlag should set help verbose true when arguments has --help"] = function(){
  let args = ['--help', '-c', '10', '-n', '2', 'file.txt', '-n12'];
  let parser = new Parser(args);
  parser.setHelpFlag();
...
test["getOptionAndCount should return object of Options"] = function(){
  let args = ['-4','-c','10','-n','9','file.txt','-n12'];
  let parser = new Parser(args,'-n','10');
  assert.deepEqual(parser.getOptionAndCount(),{ '-n': 9 , '-c': 10 });
};
